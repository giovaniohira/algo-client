...
import type { LeetCodeSession } from './auth'
...
function headers(sess: LeetCodeSession, slug?: string): Record<string, string> {
...
async function pollCheck(
  ...
  while (Date.now() < deadline) {
    ...
    if (!res.ok) throw new Error(`Check falhou: HTTP ${res.status}`)
  ...
  throw new Error('Timeout aguardando juiz do LeetCode')
}
...
export class LeetCodeAdapter {
  ...
  constructor(private sess: LeetCodeSession) {
  ...
  updateSession(sess: LeetCodeSession): void {
    this.sess = sess
    this.credential = new Credential({
      session: sess.leetcodeSession,
      csrf: sess.csrfToken
  ...
  async getProfile(): Promise<{
    ...
    if (!userStatus?.isSignedIn) throw new Error('Sessão expirada')
  ...
  async getProblem(slug: string): Promise<ProblemDetail> {
    ...
    try {
      ...
      /* ponytail: acRate opcional se stats vier malformado */
    }
  ...
  async runCode(
    ...
    if (!res.ok) throw new Error(`Run falhou: HTTP ${res.status}`)
    ...
    if (!checkId) throw new Error('Resposta de run sem interpret_id')
  ...
  async submitCode(slug: string, questionId: number, lang: string, code: string): Promise<JudgeResult> {
    ...
    if (!res.ok) throw new Error(`Submit falhou: HTTP ${res.status}`)
    const body = (await res.json()) as { submission_id?: number }
    if (!body.submission_id) throw new Error('Resposta de submit sem submission_id')
    return pollCheck(this.sess, body.submission_id, slug)
  }
}

let adapter: LeetCodeAdapter | null = null

export async function getAdapter(sess: LeetCodeSession): Promise<LeetCodeAdapter> {
  if (!adapter) {
    adapter = new LeetCodeAdapter(sess)
  } else {
